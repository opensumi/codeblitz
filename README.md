...
### 开发
#### 升级 opensumi
因为 opensumi 版本升级可能会引起 break change，因为手动升级，运行 `node scripts/upgrade-opensumi.js` 会自动拉去最新版本 opensumi，可以通过 `-v` 指定版本

#### 构建 worker-host 和 webview 资源
每个 opensumi 版本 worker-host 和 webview 可能会变更，需手动构建资源并发布到 cdn，cdn 地址由 git 管理并随 npm 包一起发布，每次发布前会检查当前 opensumi 版本资源是否已发布
