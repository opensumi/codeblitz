...
export class ChangesTreeModel extends TreeModel {
  ...
  public get onWillUpdate(): Event<void> {
    return this.onWillUpdateEmitter.event;
  }

  private flushDispatchChangeDelayer = new ThrottledDelayer<void>(
    ChangesTreeModel.DEFAULT_FLUSH_DELAY
  );

  constructor(@Optional() root: ChangeDirectory) {
    super();
    this.init(root);
  }
